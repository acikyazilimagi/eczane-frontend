  ...
  return (
    <div className={styles.cardWrapper}>
      <Box paddingTop={'10px'} fontFamily={'Roboto, sans-serif'}>
        <Stack direction="row" spacing={2} margin="0px" padding="0px 12px" alignItems="center">
          <Box padding="0 4px 0px 0px">
            <img src={healthSettings[item.typeId]?.icon} width="16px" height="16px" alt="Icon"></img>
          </Box>
          <Typography
          ...
          >
            {item.typeId === 1 ? 'Hastane' : item.typeId === 2 ? 'Eczane' : item.typeId === 4 ? 'Veteriner' : '-'}
          </Typography>
        </Stack>
      </Box>
      <Link underline="none" href={`https://www.google.com/maps/dir//${item.latitude},${item.longitude}`}>
        <Typography {...fontProps} textAlign={'left'} fontSize="20px" fontWeight="600" p="0px 12px" color={'#182151'}>
          {item.name}
        </Typography>
      </Link>
      ...
      <Stack direction="column">
        ...
        <Typography {...fontProps} marginTop={'10px!important'} textAlign={'left'} fontSize="16px" p="0px 12px">
          {item.address}
        </Typography>
          ...
          >
            <Stack {...stackProps} marginRight={'2px'} marginTop={'2px'}>
              ...
              <Link
                ...
                fontWeight={'600'}
              >
                {item.city && item.district && `${item.city} | ${item.district}`.toLocaleUpperCase()}
              </Link>
            </Stack>
            ...
            {item.phone && (
              <Stack {...stackProps} marginLeft={'2px'} marginTop={'2px'}>
                <CallIcon fontSize="12px" />
                <Link href={`tel:${item.phone}`} fontSize={'11px'} color={'#4f5fb3'}>
                  {item.phone}
                </Link>
              </Stack>
            )}
